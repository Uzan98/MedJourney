...
export class AIQuestionGeneratorService {
  private static async callGroqAPI(prompt: string): Promise<string> {
    try {
      // Chave da API Groq fixa - usar a mesma do .env.local
      const apiKey = "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";
      
      // Log para debug (não mostrar a chave completa em produção)
      console.log('Usando chave da API Groq:', apiKey ? `${apiKey.substring(0, 5)}...` : 'não definida');
      ...
      if (!response.ok) {
        ...
        console.error('Resposta da API Groq:', response.status, errorData);
